...
```bash
# 停止并删除旧容器（如果存在）
docker stop datasource-hub-mcp 2>/dev/null || true
docker rm datasource-hub-mcp 2>/dev/null || true

# 删除旧镜像（所有可能的标签）
docker rmi datasource-hub-mcp:latest datasource-hub-mcp-agent:latest 2>/dev/null || true
```
...
**构建参数说明**:
- `-t datasource-hub-mcp:latest`: 设置镜像标签
- `.`: 构建上下文为当前目录（项目根目录）

**Dockerfile 位置**: `/Users/mlamp/project/datasource-hub/Dockerfile`
...
- 使用 `uv` 进行快速依赖安装
- 从 `src/datasource-hub/` 复制源代码
- 不包含 .env 文件（通过 .dockerignore 排除）

### 4. 验证镜像构建

```bash
docker images | grep datasource-hub-mcp
```

检查输出确认镜像已成功创建，应显示类似：
```
datasource-hub-mcp    latest    [IMAGE_ID]    [时间]    [大小]
```
...
```bash
sleep 3
docker logs datasource-hub-mcp
```
...
**步骤 6.2**: 验证环境变量加载
```bash
docker exec datasource-hub-mcp printenv | grep -E "(ANTHROPIC|MCP_API_KEY|WEB_SEARCH)" | sort
```
...
**步骤 6.3**: 检查容器运行状态
```bash
docker ps --filter "name=datasource-hub-mcp" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
```
...
# 查看健康状态
docker inspect datasource-hub-mcp --format='{{.State.Health.Status}}'
```
...
📦 镜像信息:
   - 镜像名称: datasource-hub-mcp:latest
   - 镜像 ID: [显示实际 ID]
   - 镜像大小: ~300MB

🚀 容器状态:
   - 容器名称: datasource-hub-mcp
...
💡 后续操作:
   - 测试 MCP 连接: /mcp
   - 查看日志: docker logs -f datasource-hub-mcp
   - 停止服务: docker stop datasource-hub-mcp
   - 重启服务: docker restart datasource-hub-mcp
```
...
```bash
# 查看构建详细输出
docker build -t datasource-hub-mcp:latest . 2>&1 | tee build.log

# 检查构建上下文内容
docker build -t datasource-hub-mcp:latest . --progress=plain
```
...
# 手动验证环境变量
docker run --rm --env-file .env datasource-hub-mcp:latest printenv | grep ANTHROPIC
```
...
1. **数据持久化**: 数据源 JSON 文件打包在镜像中（从 `src/datasource-hub/sources/`），重启容器不会丢失数据
...
6. **项目结构要求**:
   - Dockerfile 必须在项目根目录
   - .env 文件必须在项目根目录
   - 源代码在 `src/datasource-hub/` 目录
   - pyproject.toml 和 uv.lock 在项目根目录

## 相关命令

- 查看 MCP 服务状态: `docker ps | grep datasource-hub-mcp`
- 查看实时日志: `docker logs -f datasource-hub-mcp`
- 停止服务: `docker stop datasource-hub-mcp`
- 重启服务: `docker restart datasource-hub-mcp`
- 进入容器调试: `docker exec -it datasource-hub-mcp bash`
- 查看容器环境变量: `docker exec datasource-hub-mcp printenv`
