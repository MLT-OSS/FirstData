#!/usr/bin/env python3
"""
DataSource Hub - Search and discover authoritative data sources (HTTP Mode).
...
Runs in HTTP mode (streamable-http) for deployment with Docker and remote access.
...
# Constants
# Detect sources directory location (works both in dev and Docker container)
_SCRIPT_DIR = Path(__file__).parent
if (_SCRIPT_DIR / "sources").exists():
    # Running in Docker container where sources is in /app/sources
    SOURCES_DIR = _SCRIPT_DIR / "sources"
    REPO_ROOT = _SCRIPT_DIR
else:
    # Running in development where sources is in parent directory
...
# Initialize FastMCP with HTTP mode
mcp = FastMCP(
    "datasource_hub",
    host="0.0.0.0",
    port=8000
    ...
    **Quality Assessment (6 dimensions, 1-5 stars):**
    ...
    Returns:
        ...
        **IMPORTANT OUTPUT INSTRUCTIONS:**
        - The tool returns a pre-formatted markdown table with all search results
        - You MUST output the ENTIRE returned content EXACTLY as-is, including the markdown table
        - DO NOT summarize, reformat, or re-interpret the table
        - DO NOT extract or highlight individual items from the table
        - Simply present the complete markdown output from this tool directly to the user
        - The table is already optimally formatted for readability
    ...
    try:
        ...
        scored_datasources = []
        MIN_RELEVANCE_THRESHOLD = 5.0  # Only include sources with meaningful relevance

        for ds in all_datasources:
            relevance = _calculate_relevance_score(ds, params.query)
            if relevance >= MIN_RELEVANCE_THRESHOLD:  # Filter low-relevance matches
...
class BearerAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        # Read expected API key from environment
        expected_key = os.getenv("MCP_API_KEY", "")

        # Skip auth if MCP_API_KEY is not set (for local dev)
        if not expected_key:
