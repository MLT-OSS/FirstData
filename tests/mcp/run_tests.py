...
class MCPSessionManager:
    ...
    def get_session(self) -> "MCPSession":
        ...
        if self._session is None:
            self._session = MCPSession("http://localhost:8001/mcp")
            if not self._session.initialize():
    ...
    def call_tool(self, tool_name: str, arguments: dict) -> tuple[str, float, str]:
        ...
        try:
            ...
            response = self.session.post(
                self.server_url, json=payload, headers=headers, timeout=120
            )
            ...
            return "", 0.0, f"错误: {type(e).__name__} - {e!s}"
...
def run_test_case(test_case: dict) -> dict:
    ...
    if test_case["category"] == "负面测试":
        ...
        matches = sum(
            1 for exp in expected if any(is_datasource_match(exp, rec) for rec in recommended)
        )
...
def generate_report(results: list[dict], output_file: Path):  # noqa: PLR0915
    """生成Markdown格式的测试报告"""
    ...
    for result in results:
        ...
        lines.append(
            f"**状态**: {'✅ PASS' if result['status'] == 'PASS' else '⚠️ PARTIAL' if result['status'] == 'PARTIAL' else '❌ FAIL' if result['status'] == 'FAIL' else '🔴 ERROR'}"
        )
        ...
        if result["status"] == "ERROR":
            ...
            if result.get("recommended_sources"):
                ...
                for src in result["recommended_sources"]:
                    # 检查是否匹配期望（使用改进的匹配逻辑）
                    matched = any(
                        is_datasource_match(exp, src) for exp in result.get("expected_sources", [])
                    )
...
def main():
    ...
    print("生成测试报告...")
    output_file = (
        Path(__file__).parent / f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
